...
public class NetHandler extends Thread implements PacketHandler
{
	...
	public void parsePacket(byte[] data)
	{
		...
		switch (t)
		{
			case INVALID:
			{
				Server.currentServer.frame.plog("Empfing ungŸltiges Paket");
				break;
			}
			case CATALOGS:
			{
				...
				Server.currentServer.frame.plog("Kataloge geŠndert von: " + Assistant.getSocketAddress(socket));
				break;
			}
			case CATALOG:
			{
				...
				for (int i = 0; i < Server.currentServer.catalogs.length(); i++)
				{
					try
					{
						if (Server.currentServer.catalogs.getJSONObject(i).getString("name").equals(p.getCatalog().getName()))
						{
							...
							Server.currentServer.frame.plog("Katalog " + p.getCatalog().getName() + " geŠndert von: " + Assistant.getSocketAddress(socket));
							break;
						}
					}
